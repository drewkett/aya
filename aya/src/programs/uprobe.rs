...
impl LdSoCache {
    ...
    fn parse(data: &[u8], endianness: TargetEndian) -> Result<Self, io::Error> {
        ...
        if !is_old {
            ...
            string_table_offset =
                cursor.position() as usize + num_entries as usize * mem::size_of::<u32>() * 3;
        }
        ...
        for _ in 0..num_entries {
            ...
            let key = Self::str_from_u8_nul_utf8(&cursor.get_ref()[k_pos + string_table_offset..])
                .map_err(|e| {
                    io::Error::new(
                        io::ErrorKind::InvalidData,
                ...
                .to_owned();
            let value =
                Self::str_from_u8_nul_utf8(&cursor.get_ref()[v_pos + string_table_offset..])
                    .map_err(|e| {
                        io::Error::new(
                            io::ErrorKind::InvalidData,
                            format!("Invalid ut8 string : {}", e),
...
mod tests {
    ...
    fn test_entries(strings: &str, cache: LdSoCache) {
        for string in strings.split_terminator("\n") {
            ...
            assert!(
                cache.entries.iter().any(|x| x.lib_name == lib_name),
                "lib name : {} was not inside the entries",
                lib_name
            );
            let found_paths: Vec<&CacheEntry> = cache
                .entries
                .iter()
                .filter(|e| e.lib_name == lib_name)
                .collect();
            assert!(
            ...
            assert!(
                found_paths.iter().any(|e| e.path == path),
